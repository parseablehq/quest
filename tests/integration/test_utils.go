...
func doWithRetryOn409(t *testing.T, client httpclient.HTTPClient, buildRequest func() (*http.Request, error)) *http.Response {
...
func CreateStream(t *testing.T, client pb.PBClient, dataset string) {
	...
	for {
		...
		if (!strings.Contains(output, "409") && !strings.Contains(output, "conflict") && !strings.Contains(output, "being deleted")) || time.Now().After(deadline) {
